        ...
        <ul class="space-y-4">
          ...
          <div class="container mx-auto py-8">
            <div class="flex justify-between items-center mb-6">
              ...
              <span class="text-gray-600">Dashboard / Teachers</span>
            </div>
            ...
            <div class="flex justify-between items-center py-3 bg-white mt-6" style="background-color: #f7f7fa;">
              <span class="text-sm text-gray-700">
                Showing {{ start }} to {{ end }} of {{ totalTeachers }} entries
              </span>
              <div>
...
export default {
  ...
  computed: {
    ...
    totalPages() {
      return Math.ceil(this.totalTeachers / this.perPage);
    },
    paginatedTeachers() {
    ...
    start() {
      ...
      return Math.min(this.currentPage * this.perPage, this.totalTeachers);
    }
  },
  methods: {
    toggleDropdown(menu) {
    ...
    async getTeachers() {
      const { data, error } = await supabase
        .from('profiles')
        .select('id, name, gender, department, age, phone, address')
        .eq('role', 'teacher');
