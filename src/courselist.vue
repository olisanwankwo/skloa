...
<div class="container mx-auto py-8">
        ...
        <div class="flex space-x-4 mb-6">
          <input type="text" placeholder="Search by Coursecode ..." class="px-4 py-2 border rounded-md w-full">
          ...
          <table class="min-w-full leading-normal">
            <thead>
              <tr>
                ...
                <th scope="col" class="px-5 py-3 border-b-2 border-gray-200 bg-gray-100 text-left text-xs font-semibold text-gray-600 uppercase tracking-wider">Actions</th>
              </tr>
            </thead>
            <tbody>
              <tr v-for="(course, index) in paginatedCourses" :key="course.id">
                ...
                <td class="px-5 py-5 border-b border-gray-200 bg-white text-sm">
                  <button @click="deleteCourse(course.courseid)" class="bg-red-500 text-white px-4 py-2 rounded-md">Delete</button>
                </td>
              </tr>
            </tbody>
          </table>
...
export default {
  ...
  methods: {
    ...
    async getCourses() {
      const { data, error } = await supabase.from('courses').select('courseid, name, department, level, teacher');
      if (error) {
        console.error(error);
      } else {
        this.courses = data || [];
